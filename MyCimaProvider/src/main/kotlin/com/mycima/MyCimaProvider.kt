...
class MyCima : MainAPI() {
    ...
    data class MoreEPS(val output: String)
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val recommendations = doc.select("div.Grid--WecimaPosts div.GridItem")?.mapNotNull { element ->
            element.toSearchResponse()
        }
        ...
        return if (isMovie) {
            ...
            doc.select("div.Seasons--Episodes div.Episodes--Seasons--Episodes a").apmap {
                episodes.add(
            ...
            if (moreButton.isNotEmpty()) {
                ...
                val mEPS = arrayListOf(n, n + 40, n + 80, n + 120, n + 160, n + 200, n + 240, n + 280, n + 320, 
                                     n + 360, n + 400, n + 440, n + 480, n + 520, n + 660, n + 700, n + 740, 
                                     n + 780, n + 820, n + 860, n + 900, n + 940, n + 980, n + 1020, n + 1060,
                                     n + 1100, n + 1140, n + 1180, n + 1220, totals)
                
                mEPS.apmap { it ->
                    if (it != null && it <= totals!!) {
            ...
            if (seasons.isNotEmpty()) {
                seasons.apmap { surl ->
                    ...
                    seasonsite.select("div.Seasons--Episodes div.Episodes--Seasons--Episodes a").map {
                        episodes.add(
                            newEpisode(it.attr("href")) {
                                this.name = it.text()
                                this.season = fseason
    ...
    override suspend fun loadLinks(
        ...
        doc.select(".WatchServersList > ul > li").apmap {
            ...
            if (url.contains("https://uqload.io")) {
                val doc = app.get(url).document
                val videoSource = doc.select("script:containsData(sources)").firstOrNull()?.data()?.let { scriptData ->
                    Regex("""sources:\s*\[["']([^"']+)["']]\s*,""").find(scriptData)?.groupValues?.get(1)
                }
                if (videoSource != null) {
                    callback.invoke(
                        newExtractorLink(
